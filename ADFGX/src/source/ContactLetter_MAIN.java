...
public class ContactLetter_MAIN {
	...
	public static void startRecursive(Map<String, ArrayList<String>> percHolder) throws IOException
	{
		...
		while((line = reader_ci.readLine()) != null)
		{
			...
			{	
				...
				recursive_FindPhrase(spaceList.get(i), runningString, percHolder, cipherText, cipherHolder, 0);
			}
		}
	...
	public static void recursive_FindPhrase(String checkForNext, String runningString, Map<String, ArrayList<String>> percHolder, String cipherText, 
			Map<Character, Character> cipherHolder, int depth)
	{
		...
		for(int i = 0; i < percHolder.get(checkForNext).size(); ++i)
		{
			...
			// If our holder contains a key for our current depth (sequential letter of the alphabet)
			if(cipherHolder.containsKey(cipherText.charAt(depth)))
			{
				// and if the cipher text at this letter is equal to our 
				if(cipherHolder.get(cipherText.charAt(depth)) == checkForNext.charAt(0))
				{
					goodToRun = true;
				}
				else
				{
					System.out.println(runningString + " " + cipherHolder.get(cipherText.charAt(depth)));
					goodToRun = false;
				}
			}
			...
			if(goodToRun)
			{
				if(runningString.length() < 34)
				{
					...
					if(percHolder.get(checkForNext).get(i).equals("_"))
					{
						...
						++depth;
						recursive_FindPhrase(percHolder.get("_").get(i), runningString, percHolder, cipherText, cipherHolder, depth);
					}
					else
					{
						...
						cipherHolder.put(cipherText.charAt(depth), runningString.charAt(runningString.length() - 1));
						++depth;
						recursive_FindPhrase(percHolder.get(checkForNext).get(i), runningString, percHolder, cipherText, cipherHolder, depth);
